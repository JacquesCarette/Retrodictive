...
produces this observation and for that we simply run backwars with the
...
let's understand pre-image NP etc.
...
Deutsch. The conventional statement of the problem is to determine if
a function $\finset{2} \rightarrow \finset{2}$ is constant or
...
DJ. The problem is a generalization of the previous one: the question is
...
BV. Expressing the problem as a pre-image calculation is slightly more
...
Simon.  We are given a 2-1 function $f : \finset{2^n} \rightarrow
\finset{2^n}$ with the property that there exists an $a$ such $f(x) =
...
Shor. The quantum core of the algorithm is the following. We are given
a periodic function $f(x) = a^x \mod{2^n}$ and the goal is to
determine the period. As a computation over pre-images, the problem
can be recast as follows. For an arbitrary $x$, compute the pre-image
of $f(x)$ and query it to determine the period.
