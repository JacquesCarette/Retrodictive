...
1, $x$, or $1+x$ depending on the function $f$.  The resulting
expressions are in algebraic normal form~\cite{TOKAREVA20151} where
$+$ denotes exclusive-or. In the first two cases, the observation of
the ancilla is independent of $x$, i.e. the function is constant.  In
the last two cases, the ancilla depends on $x$ (or its negation), and
...
circuit to calculate a symbolic expression. For the two constant
functions, the result was 0 or 1 indicating no dependency of the
ancilla observation on the input. For three examples of balanced
functions with $n=6$, the resulting expression was:
\begin{itemize}
\item $x_0$
...
function whose output pattern is, by design, difficult to
