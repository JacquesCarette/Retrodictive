...
~|~ f(x) = y \}$. For example, let $A = B = \finset{2^4}$ and let
$f(x) = 7^x \mod 15$, then the collection of values that $f$ maps
to~4, $\preim{f}{4}$, is the set $\{ 2, 6, 10, 14 \}$ as shown in
...
for the problem to be efficiently solvable, even by quantum
computers. Using symbolic retrodictive reasoning, the simple queries
for all the algorithms above can be answered efficiently except for
Grover and Shor.
