...
interact with the program constructs and this typically introduces
...
between barrier (1) and barrier (2) implement the quantum oracle
...
we can perform a mixed predictive and retrodictive symbolic execution
to make the flow of information conceptually clearer. We start a
forward classical simulation with one arbitrary state at barrier (1),
say $\ket{0100}$.  This state evolves to $\ket{0100}$, then
$\ket{0100}$ again, then $\ket{0110}$, and finally $\ket{0111}$. In
this case, the result of measuring the bottom two wires is
$11$. Having produced a possible measurement at barrier (3), we start
a retrodictive execution to find out what other input states might be
compatible with this future measurement. To that end, we execute the
circuit backwards with the symbolic state $\ket{x_0x_111}$; that
execution evolves to $\ket{x_0x_11(1\oplus x_1)}$, then
$\ket{x_0x_1(1\oplus x_1)(1\oplus x_1)}$, then $\ket{x_0x_1(1\oplus
  x_1)(1\oplus x_0 \oplus x_1)}$, and finally $\ket{x_0x_1(1\oplus x_0
  \oplus x_1)(1\oplus x_0 \oplus x_1)}$. At this point, we must
reconcile the constraints with the initial condition of the bottom two
wires which are both $\ket{0}$. We conclude that $1 \oplus x_0 \oplus
x_1 = 0$ or equivalently that $x_0 \neq x_1$. We have discovered that
the measurement at barrier (3) is consistent with not just the state
$\ket{01}$ we started with but also with the state $\ket{10}$. In
other words, we have $f(01) = f(10) = f(01 \oplus 11)$ and the hidden
value of $a$ is revealed to be $11$.
...
In the quantum circuit for the Simon problem instance in
Fig.~\ref{fig:Simon}, the Hadamard gates are only applied to control
wires of \cx-gates, never to wires that serve as targets of control
gates. This property is actually common in many quantum algorithms and
allows a rather simple symbolic representation of the wave
function. An initial state $\ket{0}$ followed by Hadamard will be
represented by a symbol $x$ represented as unknown boolean value
(instead of the superposition
$(1/\sqrt{2})(\ket{0}+\ket{1})$). Fig.~\ref{fig:bell} shows a circuit
to generate the Bell state $(1/\sqrt{2}) (\ket{00} + \ket{11})$. By
using the symbol $x$ for the value of the top wire after the Hadamard
gate, the input to the \cx-gate is $\ket{x0}$ which evolves to
$\ket{xx}$. The latter state, by sharing the same symbol in two
positions, accurately represents the entangled Bell state. Similarly,
for the second circuit, the state after the Hadamard gate is
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=0.7,every label/.style={rotate=40, anchor=south west}]
    ...
    measure a;
  \end{yquant*}
  \draw[line width=3pt, blue] (0,-1.6) .. controls (5,-1.8) and (11,-1.5) .. (2,-1.3);
  \draw[line width=3pt, ->, blue] (2,-1.3) .. controls (0.5,-0.8) and (7,-0.2) .. (9,-1);
  \end{tikzpicture}
