...
Let $\finset{2^{\it n}}$ denote the finite set $\{ 0,1,\ldots,(2^n-1)\}$. The
...
f(x \oplus a)$ for all~$x$; the goal is to determine $a\in \finset{2^{\it n}}$. When
...
\begin{figure}[t]
  \begin{tikzpicture}[every label/.style={rotate=40, anchor=south west}]
    ...
    box {$U_f$} (a,b);
    output {$\ket{\phi}_n$} a;
    output {$\ket{f(\phi) \oplus  \psi }_m$} b;
    \end{yquant*}
  \end{tikzpicture}
