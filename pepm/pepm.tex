...
explain why H-CCX (computationally universal)
...
\begin{enumerate}
  ...
  \item this places us in a setting combining having forward static and
    ...
    which is the domain of Partial Evaluation (PE),
...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{\label{retrofig}Retrodictive Flow}
  \end{subfigure}
...
information about the future which is, together fwith the initial
...
specification. In other words for $f : \Bool^n \rightarrow\Bool$, we wish to
generate the circuit for $g : \Bool^{n+1} \rightarrow \Bool^{n+1}$
such that for $\overline{x} : \Bool^n$, then
...
well-estbalished algorithm~\cite{soeken2016fast}. Our implementation
